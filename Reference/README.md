...
1.  **移动语义的“杀手”**：
    右值的核心价值在于“移动（Move）”而非“拷贝（Copy）”。由于 `const` 属性具有强制性，编译器无法从 `const rvalue` 中提取资源（无法修改其指针或状态），导致性能优化直接退化为最慢的**深拷贝**。
    
2.  **函数返回值的陷阱**：
    在设计接口时，通常不要为了所谓的“安全”而返回 `const` 对象（例如：`const T get_data()`）。这会导致调用者在接收该返回值时，无法触发移动构造函数来优化性能。
